...
const getGuestStoreKey = () => {
  if (typeof window !== 'undefined') {
    const cachedId = localStorage.getItem('fauxpas_guest_id');
    if (cachedId) {
      return `fauxpas_guest_store_${cachedId}`;
    }
  }
  return 'fauxpas_guest_store'; // default fallback
};

const loadGuestStore = () => {
  if (typeof window !== 'undefined') {
    const key = getGuestStoreKey();
    ...
    if (data) {
      ...
      // Reset to empty store if no data exists for this key
      guestStore = {
        projects: [],
        episodes: [],
        acts: [],
...
class GuestQueryBuilder {
  ...
  execute() {
    ...
    if (this.operation === 'select') {
      ...
      const items = Array.isArray(this.opData) ? this.opData : [this.opData];
      const upserted = items.map(item => {
        const existingIdx = guestStore[this.table].findIndex(x => x.id === item.id);
        if (existingIdx > -1) {
          const updatedItem = {
            ...guestStore[this.table][existingIdx],
            ...item,
            updated_at: new Date().toISOString()
          };
          guestStore[this.table][existingIdx] = updatedItem;
          return updatedItem;
        } else {
          const newItem = {
            id: item.id || (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2)),
      ...
      saveGuestStore();
      resultData = Array.isArray(this.opData) ? upserted : upserted[0];
...
export async function deriveCredentials(privateKey) {
  const cleanKey = (privateKey || '').trim().toLowerCase();
  if (!cleanKey) throw new Error('Private key cannot be empty');

  let hashHex;
  if (typeof window === 'undefined') {
    // Node.js env
    const crypto = require('crypto');
    hashHex = crypto.createHash('sha256').update(cleanKey).digest('hex');
  } else {
    // Browser env
  ...
  const uuidPart = hashHex.substring(0, 32);
  const writerId = [
    uuidPart.substring(0, 8),
  ...
  const password = `Pass-${hashHex.substring(32, 64)}`;
  const email = `writer-${writerId}@fauxpas.local`;

  return { writerId, email, password };
}
...
const getGuestUser = () => {
  if (typeof window !== 'undefined') {
    const cachedId = localStorage.getItem('fauxpas_guest_id');
    const username = localStorage.getItem('fauxpas_username') || 'Guest Writer';
    if (cachedId) {
      return {
        id: cachedId,
        email: `writer-${cachedId}@fauxpas.local`,
  ...
  auth: {
    ...
    signOut: () => {
      if (typeof window !== 'undefined') {
        ...
        localStorage.removeItem('fauxpas_private_key');
        localStorage.removeItem('fauxpas_guest_id');
        localStorage.removeItem('fauxpas_username');
      }
